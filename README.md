...
2. **`map[memKey]Certificate` под `sync.RWMutex`**, где `memKey = struct{ caID uint16; serial string }` (serial хранится как `string(bytes)` — Go оптимизирует это в map-key как alloc-free).
3. Sharded map с per-CA mutex.

### Решение
Вариант **2**: `map+RWMutex` с композитным ключом `(caID, serial-as-bytes)`, типизированные ошибки (`ErrNotFound`, `ErrUnavailable`), сигнатура `Get(ctx, caID, serial []byte) (Certificate, error)` — идентична будущей Postgres-реализации.

### Обоснование
- Бенчмарк `Get` под `RunParallel`: **173 ns/op, 0 allocs**. На read-heavy с редкой записью contention на `RWMutex` пренебрежимо малый.
...
- **Композитный ключ + `[]byte` serial с самого начала** — главный долгосрочный выигрыш: переход на Postgres-реализацию `Store` будет drop-in (тот же `(ctx, caID, serial)`-контракт, ровно те же `[]byte` без конверсии hex↔BYTEA). `storagetest.RunStoreContract` прогонится без изменений. Если бы ключ был просто `string`, рефакторинг бы стоил правок во всех тестах и обработчиках.
